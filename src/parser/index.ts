...
export const parsePdfObject = (buf: ArrayBuffer, offset: number): PdfObject => {
  return parseObject(new Reader(buf, offset))
}

const parseObject = (reader:Reader):PdfObject => {
  reader.skipSpace()
...
class Reader {
  ...
  skipSpace() {
    while(this.view[this.current] === 0x0a||this.view[this.current] === 0x0d || this.view[this.current] === 0x20) {
      this.current++
    }
  }
  peek(): number {
